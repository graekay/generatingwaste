document.addEventListener("DOMContentLoaded", async () => {
    try {
        npcData = await loadJSON('json/npc.json');
        locationData = await loadJSON('json/location.json');
        itemsData = await loadJSON('json/items.json');
...
function displayQuest(questText) {
    const questDisplay = document.querySelector(".result");
